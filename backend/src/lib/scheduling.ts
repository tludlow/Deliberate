...
export async function ScheduleDay(
    ...
    while (schedulingTime.isBefore(endOfDay) || schedulingTime.isSame(endOfDay)) {
        ...
        for (let [i, task] of daysTasks.entries()) {
            let splitStart = task.start_time.split(':')
            let splitEnd = task.end_time.split(':')
            let startTime = dayjs(task.day)
                .hour(splitStart[0])
                .minute(splitStart[1])
                .second(splitStart[2])
                .millisecond(0)
            let endTime = dayjs(task.day).hour(splitEnd[0]).minute(splitEnd[1]).second(splitEnd[2]).millisecond(0)

            //If a task is scheduled at this time jump to the end
            if (
                dayjs(task.day).isSame(schedulingTime, 'day') &&
                schedulingTime.isBetween(startTime, endTime, null, '[)')
            ) {
                ...
                daysTasks = daysTasks.filter((innerTask, index) => innerTask.id !== task.id)
                ...
                if (
                    schedulingTime.isAfter(
                        dayjs(`${schedulingTime.format('YYYY-MM-DD')} 17:00:00`, 'YYYY-MM-DD HH:mm:ss')
                    )
                ) {
    ...
    try {
        ...
        while (true) {
            ...
            if (now.isAfter(dayjs().hour(18).minute(0).second(0))) {
                ...
                now = now.add(1, 'hour').hour(9).minute(0).second(0).millisecond(0)
            }
            ...
            //Reset the day for tomorrows scheduling
            now = now.add(1, 'hour').hour(9).minute(0).second(0).millisecond(0)
